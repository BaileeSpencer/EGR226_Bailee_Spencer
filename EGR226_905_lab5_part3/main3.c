...
void pinint(){
    ...
    //button for reverse is on P3.6
    P3->SEL0 &= ~BIT6;
    P3->SEL1 &= ~BIT6;
    P3->DIR &= ~BIT6;  //configured button as input
    P3->REN |= BIT6;  //pull up resistor
    P3->OUT |= BIT6;
}
...
int DebounceSwitch1(void)
{
    ...
    if ((P3->IN & BIT6) == 0) //check if button 2 pushed
    { SysTick_delay(10); //pause for 10 m-sec for switch bounce
    if ((P3->IN & BIT6) == 0) //check of button 2 is still pushed
        pin_Value = 1;
    }
...
int DebounceSwitchOff(void)
{
    ...
    if (P3->IN & BIT6) //check if button 2 released
    { SysTick_delay(10); //pause for 10 m-sec for switch bounce
    if (P3->IN & BIT6) //check of button 2 is still released
        pin_Value = 1;
    }
    return pin_Value; //return 1 if pushed- 0 if not pushed
}
        ...
        if ((P3->IN & BIT6) == 0){
            if (DebounceSwitch1() ){
                while (1) {
                    ...
                    if((P2->OUT & (BIT4|BIT5|BIT6)) == 0){     //turns on red LED when all LEDs are off
                        P2->OUT |= BIT6;
                    }
                    else if(P2->OUT & BIT4){                //turns off green LED and turns on red LED when button pressed
                        P2->OUT &= ~BIT4;
                        P2->OUT |= BIT6;
                    }
                    else if(P2->OUT & BIT5){                //turns off yellow LED and turns on green LED when button pressed
                        P2->OUT &= ~BIT5;
                        P2->OUT |= BIT4;
                    }
                    else if(P2->OUT & BIT6){                //turns off red LED and turns on yellow LED when button pressed
                        P2->OUT &= ~BIT6;
                        P2->OUT |= BIT5;
                    }


                    if(P3->IN & BIT6){                     //checks for button released
                        if( DebounceSwitchOff()) {
                            break;
                        }
                    }
